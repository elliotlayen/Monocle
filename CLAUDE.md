...
- The app accent is user-selectable (Settings > General > Appearance, swatch-dot picker in `src/features/settings/components/accent-color-field.tsx`): Blue (default), Purple, Pink, Red, Orange, Yellow, Green, or System (OS accent via the CSS `AccentColor` keyword behind `@supports`; falls back to blue). The accent applies as `data-accent` on `<html>`; per-accent blocks in `src/index.css` redefine `--accent-blue` (the accent token, historically blue - name kept to avoid churn), `--ring`, and `--accent-on-color` (dark text on yellow/orange) for both themes. Persisted as `accentColor` in settings plus a `monocle-accent` localStorage fast path, owned by `theme-provider.tsx` alongside the theme. Check indicators (shadcn checkbox, dropdown checkbox items, combobox checks, calendar selection) use the accent + on-color; relationship edges, object colors, and Monaco syntax/selection hexes deliberately stay un-tinted.
...
- **Scan**: right-click a folder for "Scan for Issues" (uses the search panel's file types); `bulk_scan_cmd` streams `scan-results-batch` events into `scan-results-tab.tsx` live and returns a slim summary (no `files` vector). Cancel keeps partial results.
