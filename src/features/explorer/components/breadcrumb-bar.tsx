...
export function BreadcrumbBar() {
  ...
  if (!activeTab || activeTab.isScanResult || segments.length === 0) {
    return null;
  }
  ...
  const handleSegmentOpen = (index: number, open: boolean) => {
    ...
    const parent = useExplorerStore.getState().treeNodes.get(
      segments[index - 1].id
    );
  ...
  return (
    <div
      className="flex h-7 flex-shrink-0 items-center gap-0.5 overflow-x-auto border-b px-3 text-xs text-muted-foreground"
